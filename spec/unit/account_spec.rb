...
describe Account do
  ...
  context '#deposit' do
    ...
    it 'should instantiate a new transaction' do
      expect(transaction_class).to receive(:new).with(subject.balance)
    ...
    it 'should pass transaction instance into transactions array' do
      expect { subject.deposit(amount: 500) }.to change { subject.transactions }.from([]).to([transaction])
    end

    it 'should change balance by deposit amount' do
      expect { subject.deposit(amount: 500) }.to change { subject.balance }.from(0).to(500)
    end
  end
  ...
  context '#withdraw' do
    ...
    it 'should pass transaction instance into transactions array' do
      expect { subject.withdraw(amount: 500) }.to change { subject.transactions }.from([]).to([transaction])
    end

    it 'should change balance by withdrawal amount' do
      subject.deposit(amount: 500)
      expect { subject.withdraw(amount: 500) }.to change { subject.balance }.from(500).to(0)
    end
  end

  context '#print_statement' do
    it 'should instantiate a new statement' do
      expect(statement_class).to receive(:new).with(subject.transactions)
